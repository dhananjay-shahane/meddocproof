...
export function useDoctorNotifications(): UseDoctorNotificationsResult {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await api.get("/doctor/notifications?limit=10");
      if (res.data.success) {
  ...
  const markAsRead = useCallback(
    async (id: string) => {
      try {
        await api.put("/doctor/notifications", { notificationId: id });
  ...
  const markAllAsRead = useCallback(async () => {
    try {
      await api.put("/doctor/notifications", { markAll: true });
